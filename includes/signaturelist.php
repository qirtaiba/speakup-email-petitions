...
add_shortcode( 'signaturelist', 'dk_speakup_signatures_shortcode' );
...
add_filter( 'the_posts', 'dk_speakup_signaturelist_css' );
function dk_speakup_signaturelist_css( $posts ) {
	...
	if ( $shortcode_found ) {
		$theme = $options['signaturelist_theme'];
		 ...
		 // load default theme
		if ( $theme === 'default' ) {
			wp_enqueue_style( 'dk_speakup_signaturelist_css', plugins_url( 'speakup-email-petitions/css/signaturelist.css' ) );
		}
		// attempt to load cusom theme (petition-signaturelist.css)
			...
			if ( is_child_theme() ) {
				...
				if ( file_exists( $child_theme_path ) ) {
					wp_enqueue_style( 'dk_speakup_signaturelist_css', $child_theme_url );
				}
				// else try to load style from parent theme folder
				else {
					wp_enqueue_style( 'dk_speakup_signaturelist_css', $parent_theme_url );
				}
			}
