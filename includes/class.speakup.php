...
/**
 * Library of common methods for SpeakUp! Email Petitions plugin for WordPress
 */
class dk_speakup_SpeakUp
{
	/**
	...
	public static function progress_bar( $goal, $signatures, $max_width ) {
		...
		if ( $percent_complete < 25 ) {
			$color_class = 'dk-speakup-progressbar-low';
		}
		elseif ( $percent_complete < 75 ) {
			$color_class = 'dk-speakup-progressbar-medium';
		}
		elseif ( $percent_complete < 100 ) {
			$color_class = 'dk-speakup-progressbar-high';
		}
		else {
			$color_class = 'dk-speakup-progressbar-complete';
		}
	...
	public static function pagination( $limit, $count, $page_handle, $current_page, $base_url, $use_form ) {
		...
		$pager_html  .= '<form action="" method="post" id="dk-speakup-pager">';
		$pager_html  .= '<input type="hidden" name="dk-speakup-total-pages" id="dk-speakup-total-pages" value="' . $total_pages . '">';
		$pager_html  .= '<input type="hidden" name="dk-speakup-page" id="dk-speakup-page" value="' . $page_handle . '">';
		$pager_html  .= '<span class="displaying-num">' . $count . ' ' . __( 'items', 'dk_speakup' ) . '</span>';
		...
		if ( $use_form == true ) {
			$pager_html .= '<span class="paging-input"><input class="current-page" name="dk-speakup-paged" id="dk-speakup-paged" value="' . $current_page . '" size="1" maxlength="4" type="text"> ' . __('of') . ' <span class="total-pages">' . $total_pages . ' </span></span>';
		}
		else {
			$pager_html .= '<span class="paging-input"> ' . $current_page . ' ' . __( 'of', 'dk_speakup' ) . ' <span class="total-pages">' . $total_pages . ' </span></span>';
		}
