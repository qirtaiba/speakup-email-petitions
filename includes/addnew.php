...
function dk_speakup_addnew_page() {
	...
	switch( $action ) {
		...
		case 'create' :
			...
			$page_title  = __( 'Edit Email Petition', 'dk_speakup' );
			$nonce       = 'dk_speakup-update_petition' . $petition->id;
			$action      = 'update';
			$x_date      = $petition->get_expiration_date_components();
			$button_text = __( 'Update Petition', 'dk_speakup' );
			...
			$start_tag               = '<strong>';
			$end_tag                 = '</strong>';
			$message_text            = __( 'Petition created. Use %1$s %2$s %3$s to display in a page or post. Use %1$s %4$s %3$s to display the signatures list.', 'dk_speakup' );
		...
		case 'edit' :
			// security: ensure user has intention
			check_admin_referer( 'dk_speakup-edit_petition' . $petition->id );
		...
		case 'update' :
			// security: ensure user has intention
			check_admin_referer( 'dk_speakup-update_petition' . $petition->id );
			...
			// set up page display variables
			$page_title     = __( 'Edit New Email Petition', 'dk_speakup' );
			$nonce          = 'dk_speakup-update_petition' . $petition->id;
			$action         = 'update';
			$x_date         = $petition->get_expiration_date_components();
			$button_text    = __( 'Update Petition', 'dk_speakup' );
		...
		default :
			// set up page display variables
			$page_title     = __( 'Add New Email Petition', 'dk_speakup' );
			$nonce          = 'dk_speakup-create_petition';
			$action         = 'create';
			$x_date         = $petition->get_expiration_date_components();
			$button_text    = __( 'Create Petition', 'dk_speakup' );
			$message_update = '';
